...
static func draw_soldier(
	...
	var pack_x := -facing * 16.0
	node.draw_rect(Rect2(pack_x - 6.0, -44.0, 12.0, 30.0), pack_col)
	node.draw_rect(Rect2(pack_x - 8.0, -44.0, 16.0, 6.0), pack_col.darkened(0.2))
	...
	if not Gostek.has_frame(node):
		shoulder = Vector2(facing * 6.0, -38.0)
	...
	if muzzle_t > 0.0:
		var flash_pos: Vector2 = barrel_end + aim_dir * 6.0
		var m := muzzle_t
		node.draw_circle(flash_pos, 6.0 + m * 60.0, Color(1.0, 0.45, 0.15, m * 0.35))
		node.draw_circle(flash_pos, 4.8 + m * 44.0, Color(1.0, 0.8, 0.35, m * 0.6))
		node.draw_circle(flash_pos, 3.2 + m * 28.0, Color(1.0, 1.0, 0.7, m * 0.85))

	# HP + optional fuel bar.
	node.draw_rect(Rect2(-32.0, HP_BAR_Y, 64.0, 8.0), Color(0.0, 0.0, 0.0, 0.55))
	node.draw_rect(Rect2(-32.0, HP_BAR_Y, 64.0 * clampf(health / 100.0, 0.0, 1.0), 8.0), Color(0.9, 0.2, 0.2))
	if show_fuel:
		node.draw_rect(Rect2(-32.0, HP_BAR_Y + 10.0, 64.0, 6.0), Color(0.0, 0.0, 0.0, 0.55))
		node.draw_rect(Rect2(-32.0, HP_BAR_Y + 10.0, 64.0 * clampf(fuel / 100.0, 0.0, 1.0), 6.0), Color(0.3, 0.7, 1.0))
...
static func _draw_jet_flame(node: CanvasItem, facing: float) -> void:
	...
	var origin: Vector2 = Vector2(-facing * 16.0, 6.0)
	var length := 44.0 + sin(t * 26.0) * 9.0
	var wobble := cos(t * 18.0) * 2.4
	var w0 := 10.0 + wobble
	var w1 := 6.8 + wobble * 0.7
	var w2 := 3.8
