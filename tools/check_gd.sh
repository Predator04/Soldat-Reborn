#!/usr/bin/env bash
# Soldat-Reborn compile check — PostToolUse hook (fires on every Edit/Write).
# v2.1.148 notes: glob matchers like Edit(*.gd) DON'T fire, and $CLAUDE_FILE_PATHS
# is not set — so we detect .gd edits via git status instead.
# Full headless boot is the ONLY reliable check: `--headless --import` is a no-op
# on a warm cache, and `--check-only --script` false-positives on autoload names.
...
# Skip if no .gd file is currently modified/staged/untracked — avoids an 11s
# boot on .md/.tscn/.json edits. New .gd files show as "??" and still match.
if ! git status --porcelain 2>/dev/null | grep -qE '\.gd$'; then
  exit 0
fi
